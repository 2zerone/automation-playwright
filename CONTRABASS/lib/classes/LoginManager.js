...
class LoginManager extends BaseManager {
  ...
  async processLogin(config) {
    try {
      console.log('🔐 CONTRABASS 로그인 프로세스 시작...');
      ...
      console.log('✅ CONTRABASS 로그인 프로세스 완료');
      return { success: true, message: 'CONTRABASS 로그인 완료' };
      
    } catch (error) {
      console.error('CONTRABASS 로그인 실패:', error.message);
      throw error;
    }
  }
  ...
  async navigateToLoginPage() {
    console.log('🌐 CONTRABASS 로그인 페이지 접근 중...');
    ...
    // 로그인 페이지 접근 스크린샷
    await this.captureScreenshot('CONTRABASS 로그인 페이지 접근');
    
    console.log('✅ CONTRABASS 로그인 페이지 접근 완료');
  }
  ...
  async fillLoginCredentials(config) {
    ...
    // 로그인 정보 입력 스크린샷
    await this.captureScreenshot('CONTRABASS 로그인 정보 입력');
  ...
  async submitLoginAndVerify() {
    ...
    await this.page.waitForTimeout(1000);
    try {
      // Service Desk가 나타날 때까지 대기 (최대 30초)
    ...
    await this.captureScreenshot('CONTRABASS 로그인 성공');
