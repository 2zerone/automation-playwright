...
console.log('🎻 VIOLA Electron 시나리오 실행기\n');

// VIOLA 시나리오 구조 레포트 출력 함수
function printViolaReport(scenarioId) {
    console.log(`\n📊 VIOLA 시나리오 ${scenarioId} 구조 레포트`);
    console.log('=' .repeat(60));
    
    // VIOLA 시나리오 목록
    const violaScenarios = [
        'VIOLA 클러스터 생성 및 확인',
        '외부접속 서비스 트래픽 모니터링',
        '서비스 어카운트 클러스터 외부접근',
        '카탈로그 허브(Helm_Chart) 네트워크 통신 제한',
        '카탈로그 허브(YAML) 오토스케일링 및 알림',
        'GPU 파드 생성 및 모니터링',
        '일반 배포 파이프라인 생성',
        '긴급 배포 파이프라인 생성'
    ];
    ...
    violaScenarios.forEach((scenario, index) => {
...
async function runScenario(scenarioId) {
        ...
        if (fs.existsSync(globalTestResultsPath)) {
            try {
                ...
                Object.keys(rawData).forEach(key => {
                    // 유효한 키 패턴: 'viola-scenario-N', 'scenario-N', 'N' (N은 숫자)
                    const validKeyPattern = /^(viola-scenario-\d+|scenario-\d+|\d+)$/;
    ...
    try {
        console.log(`🚀 VIOLA 시나리오 ${scenarioId} 실행을 시작합니다...\n`);
        ...
        try {
            ...
            if (fs.existsSync(recordingSettingsPath)) {
                ...
                console.log(`📹 VIOLA 시나리오 ${scenarioId} 녹화 설정: ${recordingEnabled ? '활성화' : '비활성화'}`);
            }
        ...
        try {
            ...
            global.violaTestOutput = playwrightOutput;
            ...
            global.violaExecutionError = planAError;
            // 오류를 던지지 않고 계속 진행
            console.log(`❌ VIOLA 시나리오 ${scenarioId} 실행 중 오류 발생`);
        }
        
        // VIOLA 동적 테스트 결과 파싱 (성공/실패 모든 경우에 적용)
        console.log(`📊 VIOLA 동적 테스트 결과 파싱 시작`);
        ...
        // 브라우저 강제 종료 감지: global.violaExecutionError가 있으면 test-results.json을 신뢰하지 않음
        const isBrowserForceClosedFromError = !!global.violaExecutionError;
        if (isBrowserForceClosedFromError) {
            console.log(`⚠️ VIOLA 브라우저 강제 종료 감지 (violaExecutionError 존재), test-results.json 무시`);
        }
        
        if (fs.existsSync(testResultsPath) && !isBrowserForceClosedFromError) {
            console.log(`📊 VIOLA Playwright 결과 파일 발견: ${testResultsPath}`);
            ...
            console.log(`📊 VIOLA 시나리오 ${scenarioId} 파일 패턴 검색 중...`);
            for (const pattern of patterns) {
                scenarioSuite = testResultsData.suites?.find(suite => 
                    suite.file && (suite.file === pattern || suite.file.includes(pattern))
                );
                if (scenarioSuite) {
                    console.log(`✅ VIOLA 시나리오 발견 (패턴: ${pattern}, 실제 파일: ${scenarioSuite.file})`);
                    break;
                } else {
                    console.log(`❌ VIOLA 패턴 매칭 실패: ${pattern}`);
                }
            }
            
            // 디버깅: 사용 가능한 모든 suite 파일 출력
            if (!scenarioSuite && testResultsData.suites) {
                console.log(`📊 VIOLA 사용 가능한 suite 파일들:`);
            ...
            if (scenarioSuite) {
                console.log(`📊 VIOLA 시나리오 ${scenarioId} 테스트 결과 발견`);
                ...
                console.log(`📊 VIOLA 발견된 테스트 스펙: ${allSpecs.length}개`);
                ...
                allSpecs.forEach(spec => {
                    ...
                    if (spec.tests && spec.tests.length > 0) {
                        spec.tests.forEach(test => {
                            ...
                            console.log(`🔍 [DEBUG] test.title: "${test.title}", 최종 testName: "${testName}"`);
                            
                            // describe 블록 이름 필터링 (시나리오 전체 이름 제외)
                            if (testName.includes('VIOLA Scenario') || 
                ...
                console.log(`📊 VIOLA 전체 테스트: ${allTests.length}개, results 있음: ${allTests.filter(t => t.hasResults).length}개`);
                ...
                if (firstTestWithResults && firstTestWithResults.result.startTime) {
                    ...
                    console.log(`📊 VIOLA 시나리오 시작 시간 (첫 번째 테스트): ${scenarioStartTime.toISOString()}`);
                } else {
                    ...
                    console.log(`📊 VIOLA 시나리오 시작 시간 (추정): ${scenarioStartTime.toISOString()}, 전체 duration: ${totalTestDuration}ms`);
                }
                ...
                if (testResults.length > 0 && overallStatus !== 'pass') {
                    console.log(`📊 VIOLA 브라우저 강제 종료 지점 탐색 시작 (총 ${testResults.length}개 테스트)`);
                    console.log(`📊 VIOLA testResults 상세:`, testResults.map((t, i) => `[${i}] ${t.name}: ${t.status} (hasResults: ${t.hasResults})`).join(', '));
                    
                    // results가 없는 테스트가 있는지 확인
                    const hasTestsWithoutResults = testResults.some(t => !t.hasResults);
                    console.log(`📊 VIOLA hasTestsWithoutResults: ${hasTestsWithoutResults}`);
                    
                    if (hasTestsWithoutResults) {
                        console.log(`⚠️ VIOLA results가 없는 테스트 발견 - 브라우저 강제 종료 지점 탐지`);
                        overallStatus = 'fail';  // 브라우저 강제 종료로 fail 처리
                        console.log(`📊 VIOLA overallStatus를 fail로 변경`);
                        ...
                        for (let i = 0; i < testResults.length; i++) {
                            ...
                            if (testCase.hasResults) {
                                ...
                                if (testCase.status === 'fail' || hasBrowserClosedError) {
                                    failIndex = i;
                                    console.log(`⚠️ VIOLA 브라우저 강제 종료 지점 감지 (실제 fail/error): ${testCase.name} (인덱스: ${i})`);
                                    break;
                                }
                            }
                        }
                        ...
                        if (failIndex === -1) {
                            ...
                            if (lastResultsIndex >= 0) {
                                ...
                                console.log(`⚠️ VIOLA 브라우저 강제 종료 지점 (추정): ${testResults[failIndex].name} (인덱스: ${failIndex}, 마지막 results 있는 테스트)`);
                            } else if (testResults.length > 0) {
                                // results가 없는 테스트만 있는 경우, 첫 번째 테스트를 fail로
                                failIndex = 0;
                                console.log(`⚠️ VIOLA 브라우저 강제 종료 지점 (추정): ${testResults[failIndex].name} (인덱스: ${failIndex}, 첫 번째 테스트)`);
                            }
                        }
                        ...
                        console.log(`📊 VIOLA 브라우저 강제 종료 처리 완료: ${testResults.filter(t => t.status === 'pass').length}개 성공, ${testResults.filter(t => t.status === 'fail').length}개 실패, ${testResults.filter(t => t.status === 'not-test').length}개 미수행`);
                    } else if (global.violaExecutionError) {
                        // results가 모두 있지만 execSync 오류가 발생한 경우
                        console.log(`📊 VIOLA execSync 오류 감지 - error 메시지나 fail 상태로 찾기`);
                    overallStatus = 'fail';
                        ...
                        for (let i = 0; i < testResults.length; i++) {
                            ...
                            if (!failPointFound) {
                                ...
                                if (testCase.status === 'fail' || hasBrowserClosedError) {
                                    console.log(`⚠️ VIOLA 브라우저 강제 종료 지점 감지: ${testCase.name}`);
                                    failPointFound = true;
                                    // 이미 fail 상태
                                }
                            } else {
                ...
                console.log(`📊 VIOLA 파싱 결과: ${testResults.length}개 테스트, 상태: ${overallStatus}, totalDuration: ${totalDuration}ms`);
                ...
                if (testResults.length > 0) {
                    ...
                    console.log(`📊 VIOLA duration 계산: ${totalDuration}ms → ${formattedDuration}`);
                    console.log(`📊 VIOLA testResults 상세 duration: ${testResults.map(t => `${t.name}=${t.duration}ms`).join(', ')}`);
                    ...
                    global.testResults.set(`viola-scenario-${scenarioId}`, testResultsData);
                    console.log(`📊 VIOLA 성공 케이스 동적 파싱 결과 저장 완료: ${testResults.length}개 테스트 케이스`);
                    
                    // 공통 함수로 저장
                    saveGlobalTestResults(scenarioId, testResultsData, 'VIOLA 성공 케이스 동적 결과 저장 완료');
                }
            } else {
                console.log(`⚠️ VIOLA 시나리오 ${scenarioId} 테스트 결과를 찾을 수 없음`);
            }
        } else {
            console.log(`⚠️ VIOLA test-results.json 파일이 존재하지 않음: ${testResultsPath}`);
            
            // 백업 파일 확인
            const backupPath = path.join(__dirname, 'playwright-report', 'test-results.backup.json');
            if (fs.existsSync(backupPath)) {
                console.log(`📦 VIOLA 백업 파일 발견, 백업 파일로 파싱 시도: ${backupPath}`);
                try {
                    ...
                    if (scenarioSuite) {
                        console.log(`✅ VIOLA 백업 파일에서 시나리오 발견`);
                        ...
                        console.log(`📊 VIOLA 백업에서 ${allTests.length}개 테스트 발견, 모두 실패로 간주`);
                        ...
                        if (testResults.length > 0) {
                            ...
                            console.log(`⚠️ VIOLA 첫 번째 테스트를 fail로 설정: ${testResults[0].name}`);
                        }
                        ...
                        const formattedDuration = `${minutes}분 ${seconds}초`;
                        console.log(`📊 VIOLA 백업 파일 기반 duration 계산: ${actualDurationMs}ms → ${formattedDuration}`);
                        ...
                        global.testResults.set(`viola-scenario-${scenarioId}`, testResultsData);
                        saveGlobalTestResults(scenarioId, testResultsData, 'VIOLA 백업 파일 기반 실패 결과 저장 완료');
                    }
                } catch (backupError) {
                    console.error(`❌ VIOLA 백업 파일 파싱 실패: ${backupError.message}`);
                }
            } else {
                console.log(`⚠️ VIOLA 백업 파일도 없음, 실패 정보만 저장`);
            }
        }
            ...
            if (terminalOutput) {
                ...
                const extractTestCasesFromSpecFile = (scenarioId) => {
                    try {
                        ...
                        console.log(`📂 VIOLA 시나리오 ${scenarioId} spec 파일 경로: ${specFilePath}`);
                        
                        if (!fs.existsSync(specFilePath)) {
                            console.log(`⚠️ VIOLA 시나리오 ${scenarioId} spec 파일이 존재하지 않음`);
                            return [];
                        }
                        ...
                        if (!match) {
                            console.log(`⚠️ VIOLA 시나리오 ${scenarioId} spec 파일에서 allPlannedTestCases를 찾을 수 없음`);
                            return [];
                        }
                        ...
                        console.log(`✅ VIOLA 시나리오 ${scenarioId} spec 파일에서 ${testCases.length}개 테스트 케이스 추출`);
                        return testCases;
                    } catch (error) {
                        console.log(`❌ VIOLA 시나리오 ${scenarioId} spec 파일 파싱 실패: ${error.message}`);
                        return [];
                    }
                };
                ...
                if (testCaseNames.length > 0) {
                    ...
                    console.log(`📊 VIOLA 시나리오 ${scenarioId} 동적 테스트 패턴: ${testPatterns.length}개`);
                } else {
                    // fallback: 기본 패턴
                    console.log(`⚠️ VIOLA 시나리오 ${scenarioId} spec 파일에서 추출 실패, 기본 패턴 사용`);
                    testPatterns = [
                ...
                console.log(`📊 VIOLA 시나리오 ${scenarioId} 최종 테스트 패턴 수: ${testPatterns.length}개`);
                
                // 실패 감지 (브라우저 강제 종료 또는 일반 실패)
                const browserClosed = terminalOutput.includes('Target page, context or browser has been closed');
                const hasExecutionError = global.violaExecutionError !== null && global.violaExecutionError !== undefined;
                let failurePoint = -1;
                ...
                if (browserClosed) {
                    console.log(`❌ VIOLA 터미널에서 브라우저 강제 종료 감지`);
                        ...
                        console.log(`❌ VIOLA 터미널에서 execSync 오류 감지 - 실패 지점 찾기`);
                    }
                    ...
                    if (screenshotErrorMatch) {
                        ...
                        console.log(`🔍 VIOLA 스크린샷 에러에서 실패 단계 발견: "${failedStepName}"`);
                        
                        // testPatterns에서 해당 이름 찾기
                        for (let i = 0; i < testPatterns.length; i++) {
                            if (testPatterns[i].name === failedStepName) {
                                failureFromScreenshotError = i;
                                console.log(`✅ VIOLA 스크린샷 에러 단계 매칭: ${testPatterns[i].name} (인덱스 ${i})`);
                                break;
                            }
                        }
                    }
                        ...
                        if (testPatterns[i].pattern.test(terminalOutput)) {
                            ...
                            if (durationMatch) {
                                ...
                                if (duration < 200) {
                                    ...
                                    console.log(`⚠️ VIOLA 비정상적으로 짧은 duration 발견: ${testPatterns[i].name} (${duration}ms)`);
                                    break;
                                }
                            }
                        }
                    ...
                    if (failureFromScreenshotError >= 0) {
                        ...
                        console.log(`❌ VIOLA 스크린샷 에러 단계를 실패로 간주: ${testPatterns[failurePoint].name} (인덱스 ${failurePoint})`);
                    } else if (lastPassIndexWithShortDuration >= 0) {
                        failurePoint = lastPassIndexWithShortDuration;
                        console.log(`❌ VIOLA 비정상적으로 짧은 duration 단계를 실패로 간주: ${testPatterns[failurePoint].name} (인덱스 ${failurePoint})`);
                    } else {
                        // PASS 없는 첫 단계를 실패로 간주
                        for (let i = 0; i < testPatterns.length; i++) {
                            if (!testPatterns[i].pattern.test(terminalOutput)) {
                                failurePoint = i;
                                console.log(`❌ VIOLA PASS 없는 첫 단계를 실패로 간주: ${testPatterns[i].name} (인덱스 ${i})`);
                                break;
                            }
                        }
                    }
                ...
                testPatterns.forEach((test, index) => {
                    ...
                    if ((browserClosed || hasExecutionError) && index === failurePoint) {
                        ...
                        if (durationMatch1) {
                            ...
                            console.log(`⏱️ VIOLA failurePoint duration 추출 (패턴1): ${test.name} = ${duration}ms`);
                        }
                        ...
                        if (!durationExtracted) {
                            ...
                            if (durationMatch2) {
                                ...
                                durationExtracted = true;
                                console.log(`⏱️ VIOLA failurePoint duration 추출 (패턴2): ${test.name} = ${duration}ms`);
                            }
                        }
                        ...
                        if (!durationExtracted) {
                            ...
                            if (durationMatch3) {
                                ...
                                durationExtracted = true;
                                console.log(`⏱️ VIOLA failurePoint duration 추출 (패턴3): ${test.name} = ${duration}ms`);
                            }
                        }
                        
                        if (!durationExtracted) {
                            duration = 100; // 비정상 종료 시 기본값
                            console.log(`⚠️ VIOLA failurePoint duration 추출 실패, 기본값 사용: ${test.name} = ${duration}ms`);
                        }
                        ...
                        } else if (hasExecutionError && global.violaExecutionError) {
                            // global.violaExecutionError에서 핵심 에러 메시지 추출
                            const err = global.violaExecutionError;
                ...
                console.log(`📊 VIOLA 터미널 파싱 완료: ${testResults.length}개 테스트, 상태: ${overallStatus}, totalDuration: ${totalDuration}ms`);
                ...
                if (testResults.length > 0) {
                    ...
                    const seconds = totalSeconds % 60;
                    const formattedDuration = `${minutes}분 ${seconds}초`;
                    console.log(`📊 VIOLA 터미널 파싱 duration 계산: ${totalDuration}ms → ${formattedDuration}`);
                    ...
                    global.testResults.set(`viola-scenario-${scenarioId}`, testResultsData);
                    console.log(`📊 VIOLA 터미널 파싱 결과 저장 완료: ${testResults.length}개 테스트 케이스`);
                    
                    // 공통 함수로 저장
                    saveGlobalTestResults(scenarioId, testResultsData, 'VIOLA 터미널 파싱 결과 저장 완료');
                }
            }
        ...
        try {
            ...
            console.log(`📊 VIOLA output 길이: ${output.length}자`);
            if (output.length > 0) {
                console.log(`📊 VIOLA output 샘플: ${output.substring(0, 300)}...`);
            }
        } catch (e) {
        ...
        if (output.includes('Target page, context or browser has been closed')) {
            console.log(`❌ VIOLA 브라우저 강제 종료 감지`);
            ...
            console.log(`📊 VIOLA 추정 완료 단계: ${estimatedCompletedSteps}개 (실행시간: ${executionTime}ms, 총 단계: ${testResults.length}개)`);
            ...
            console.log(`📊 VIOLA 브라우저 강제 종료 처리 완료: ${testResults.filter(t => t.status === 'pass').length}개 성공, ${testResults.filter(t => t.status === 'fail').length}개 실패, ${testResults.filter(t => t.status === 'not-test').length}개 미실행`);
        }
        
        console.log(`📊 VIOLA 최종 파싱 결과: ${testResults.length}개 테스트 케이스, 상태: ${overallStatus}, totalDuration: ${totalDuration}ms`);
        ...
        if (testResults.length > 0) {
            ...
            const seconds = totalSeconds % 60;
            const formattedDuration = `${minutes}분 ${seconds}초`;
            console.log(`📊 VIOLA 최종 파싱 duration 계산: ${totalDuration}ms → ${formattedDuration}`);
            ...
            global.testResults.set(`viola-scenario-${scenarioId}`, testResultsData);
            console.log(`📊 VIOLA 동적 파싱 결과 저장 완료: ${testResults.length}개 테스트 케이스`);
            
            // 공통 함수로 저장
            saveGlobalTestResults(scenarioId, testResultsData, 'VIOLA 브라우저 강제 종료 결과 저장 완료');
        }
        ...
        console.log('\n📊 새로운 커스텀 리포트 생성 중...');
        try {
            // VIOLA의 report-generator.js를 직접 호출하여 커스텀 리포트 생성
            ...
            console.log(`📊 VIOLA successTestResultsData 생성: status=${overallStatus}, testCases=${testResults.length}개`);
            ...
            // 실제 Playwright 테스트 결과 파싱 (VIOLA 독립적)
            try {
                const testResultsPath = path.join(__dirname, 'playwright-report/test-results.json');
            ...
            global.testResults.set(`viola-scenario-${scenarioId}`, successTestResultsData);
            ...
            global.testResults.set(`viola-scenario-${scenarioId}`, finalTestResults);
            ...
            console.log('✅ VIOLA 커스텀 리포트 생성 완료');
            ...
            console.log('✅ VIOLA 시나리오 목록 업데이트 완료');
            ...
            console.log(`✅ VIOLA 시나리오 ${scenarioId} 완료 플래그 생성: ${completeFlagPath}`);
            
            // 실제 상태가 fail이나 stopped면 종료 코드 1로 종료
            if (overallStatus === 'fail' || overallStatus === 'stopped') {
                console.log(`⚠️ VIOLA 시나리오 ${scenarioId} 실패 상태로 종료: ${overallStatus}`);
            }
            ...
            if (overallStatus === 'fail' || overallStatus === 'stopped') {
                console.log(`❌ VIOLA 시나리오 ${scenarioId} ${overallStatus} 상태로 종료 (exit code: 1)`);
                process.exit(1);
            }
            
        } catch (error) {
            console.error('❌ VIOLA 커스텀 리포트 생성 실패:', error.message);
        }
        ...
        try {
            ...
            try {
                console.log(`📊 VIOLA 실제 실행 로그 파싱 시작`);
                ...
                if (error && error.message) {
                    ...
                    console.log(`📊 VIOLA 오류 메시지: ${errorMessage}`);
                    
                    // 브라우저 종료 오류가 있는 경우 (실제 실행 로그에서 확인)
                    // 실제 실행 로그에서 "❌ 로그인 실행: FAIL"과 같은 패턴이 있었음
                    console.log(`❌ VIOLA 브라우저 종료 감지 - 실행 로그에서 실패한 테스트 찾기`);
                    
                    // VIOLA 동적 테스트 결과 파싱 (TROMBONE 방식 참고)
                    console.log(`📊 VIOLA 동적 테스트 결과 파싱 시작`);
                ...
                // 시나리오 파일 패턴 매칭 (VIOLA 방식) - 실제 파일 경로에 맞게 수정
                let scenarioSuite = null;
                const patterns = [
                    `scenario/scenario-${scenarioId}.spec.js`,  // 실제 경로
                    `tests/scenario/scenario-${scenarioId}.spec.js`,
                ...
                if (!scenarioSuite && testResultsData.suites) {
                    ...
                    testResultsData.suites.forEach((suite, index) => {
                        ...
                        if (!scenarioSuite) {
                            console.log(`📊 VIOLA 시나리오를 찾을 수 없음. 사용 가능한 파일들:`);
                            testResultsData.suites?.forEach((suite, index) => {
                        ...
                        if (suite.specs) {
                            ...
                            const allSpecs = findAllSpecs(scenarioSuite);
                            console.log(`📊 VIOLA [실패 케이스] 발견된 테스트 스펙: ${allSpecs.length}개`);
                            ...
                            console.log(`📊 VIOLA [실패 케이스] 전체 테스트: ${allTests.length}개, results 있음: ${allTests.filter(t => t.hasResults).length}개`);
                            ...
                            const hasTestsWithoutResults = testResults.some(t => !t.hasResults);
                            if (hasTestsWithoutResults) {
                                console.log(`⚠️ VIOLA [실패 케이스] results가 없는 테스트 발견 - 브라우저 강제 종료 지점 탐지`);
                                ...
                                if (lastResultsIndex >= 0 && lastResultsIndex + 1 < testResults.length) {
                                    ...
                                    console.log(`⚠️ VIOLA [실패 케이스] 브라우저 강제 종료 지점: ${testResults[failIndex].name} (인덱스: ${failIndex}, 마지막 results 있는 테스트 다음)`);
                                } else if (lastResultsIndex >= 0) {
                                    // 모든 테스트가 results를 가지고 있는데 hasTestsWithoutResults가 true?
                                    // 이 경우는 마지막 테스트를 fail로
                                    failIndex = lastResultsIndex;
                                    console.log(`⚠️ VIOLA [실패 케이스] 브라우저 강제 종료 지점: ${testResults[failIndex].name} (인덱스: ${failIndex}, 마지막 테스트)`);
                                } else if (testResults.length > 0) {
                                    // results가 없는 테스트만 있는 경우, 첫 번째 테스트를 fail로
                                    failIndex = 0;
                                    console.log(`⚠️ VIOLA [실패 케이스] 브라우저 강제 종료 지점: ${testResults[failIndex].name} (인덱스: ${failIndex}, 첫 번째 테스트)`);
                                }
                            ...
                            console.log(`📊 VIOLA 파싱 결과: ${testResults.length}개 테스트, 상태: ${overallStatus}`);
                        } else {
                            console.log(`⚠️ VIOLA 시나리오 ${scenarioId} 테스트 결과를 찾을 수 없음`);
                        }
                    } else {
                    ...
                    // VIOLA 테스트 결과가 없는 경우에도 동적 결과만 사용
                    if (testResults.length === 0) {
                        console.log(`📊 VIOLA 테스트 결과가 없음 - 동적 파싱 실패`);
                        // 하드코딩 제거 - 동적 결과만 사용
                        overallStatus = 'fail';
                        totalDuration = 0;
                    }
                    ...
                    const seconds = totalSeconds % 60;
                    const formattedDuration = `${minutes}분 ${seconds}초`;
                    console.log(`📊 VIOLA violaTestResultsData duration 계산: ${totalDuration}ms → ${formattedDuration}`);
                    
                    // VIOLA 테스트 결과 데이터 생성 (커스텀 리포트용)
                    const violaTestResultsData = {
                        status: overallStatus,
                        duration: formattedDuration,
                        startTime: new Date().toISOString(),
                        endTime: new Date().toISOString(),
                    ...
                    console.log(`📊 VIOLA 테스트 결과 데이터 생성 완료: ${testResults.length}개 테스트 케이스, 상태: ${overallStatus}`);
                    ...
                    console.log(`📊 VIOLA 실제 실행 로그 기반 파싱 완료: ${testResults.testCases.length}개 테스트 케이스`);
                } else {
                    console.log(`⚠️ VIOLA 오류 메시지가 없음 - 기본 파싱 시도`);
                    
            // ⚠️ 보수적 접근: 첫 번째 파싱과 두 번째 파싱 결과를 비교하여 더 나은 결과 선택
            const firstParseCount = testResults.testCases.length;
            console.log(`📊 VIOLA 첫 번째 파싱 결과: ${firstParseCount}개 테스트 케이스`);
            ...
            if (fs.existsSync(testResultsPath)) {
                ...
                const scenarioFilePattern = `scenario/scenario-${String(scenarioId)}.spec.js`;
                        ...
                        if (scenarioSuite) {
                            ...
                            const allSpecs = findAllSpecs(scenarioSuite);
                            console.log(`📊 VIOLA 발견된 스펙 수: ${allSpecs.length}`);
                                ...
                                if (spec.tests && spec.tests.length > 0) {
                                    spec.tests.forEach(test => {
                                        if (test.results && test.results.length > 0) {
                                            ...
                                            console.log(`📊 VIOLA 테스트 결과: ${result.status}, 제목: ${spec.title}`);
                                            
                                            // 실제 결과 상태에 따라 상태 설정
                                            let status = 'pass';
                                            if (result.status === 'failed' || result.status === 'timedOut') {
                                                status = 'fail';
                                                console.log(`❌ VIOLA 실패한 테스트 발견: ${spec.title}`);
                                            } else if (result.status === 'skipped') {
                                                status = 'skip';
                            ...
                            console.log(`📊 VIOLA Playwright 파싱 결과: ${secondParseCount}개 테스트 케이스`);
                ...
                console.error('❌ VIOLA 테스트 결과 파싱 중 오류:', parseError.message);
            }
            ...
            global.testResults.set(`viola-scenario-${scenarioId}`, testResults);
            ...
            console.error('❌ VIOLA 실패 케이스 커스텀 리포트 생성 실패:', reportError.message);
        }
