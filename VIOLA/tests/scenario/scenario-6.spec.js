...
test.describe.serial('시나리오 6: testaa', () => {
  let page;
  let browser;
  let config;
  let managers;
  ...
  test.beforeAll(async ({ browser: newBrowser }) => {
    const result = await setupScenarioBeforeAll(newBrowser, 6, initializeManagers);
  ...
  test.afterAll(async () => {
    await setupScenarioAfterAll(page, browser, 6, '시나리오 6: testaa', 'VIOLA');
  });
  ...
  test('Fill username', async () => {
    await test.step('Fill username', async () => {
      await runTestStep('Fill username', async () => {
        ...
        await managers.autoRecorded_testaa.fillUsernameTextbox(config);
  ...
  test('Fill password', async () => {
    await test.step('Fill password', async () => {
      await runTestStep('Fill password', async () => {
        ...
        await managers.autoRecorded_testaa.fillPasswordTextbox(config);
  ...
  test('Click login button', async () => {
    await test.step('Click login button', async () => {
      await runTestStep('Click login button', async () => {
        ...
        await managers.autoRecorded_testaa.clickLoginButton(config);
