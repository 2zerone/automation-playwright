...
import AutoRecorded_testtt from '../../lib/classes/AutoRecorded_testtt.js';
...
function initializeManagers(page) {
  return {
    autoRecorded_testtt: new AutoRecorded_testtt(page)
  };
}

test.describe.serial('시나리오 5: testtt', () => {
  let page;
  let browser;
  let config;
  let managers;
  ...
  test.beforeAll(async ({ browser: newBrowser }) => {
    const result = await setupScenarioBeforeAll(newBrowser, 5, initializeManagers);
  ...
  test.afterAll(async () => {
    await setupScenarioAfterAll(page, browser, 5, '시나리오 5: testtt', 'VIOLA');
  });
  ...
  test('Navigate to page', async () => {
    await test.step('Navigate to page', async () => {
      await runTestStep('Navigate to page', async () => {
        ...
        await managers.autoRecorded_testtt.navigateToPage(config);
  ...
  test('Fill username', async () => {
    await test.step('Fill username', async () => {
      await runTestStep('Fill username', async () => {
        ...
        await managers.autoRecorded_testtt.fillUsernameTextbox(config);
  ...
  test('Fill password', async () => {
    await test.step('Fill password', async () => {
      await runTestStep('Fill password', async () => {
        ...
        await managers.autoRecorded_testtt.fillPasswordTextbox(config);
  ...
  test('Click login button', async () => {
    await test.step('Click login button', async () => {
      await runTestStep('Click login button', async () => {
        ...
        await managers.autoRecorded_testtt.clickLoginButton(config);
