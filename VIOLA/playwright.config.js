...
const root = path.resolve(__dirname); // VIOLA 폴더 경로

/**
 * @see https://playwright.dev/docs/test-configuration
 */
export default defineConfig({
  // 테스트 디렉토리 설정 - VIOLA 폴더 내의 tests 디렉토리 참조
  testDir: path.join(root, 'tests').replace(/\\/g, '/'), // VIOLA/tests
  ...
  // 녹화 파일 출력 디렉토리 설정 (VIOLA 전용)
  outputDir: path.join(root, 'test-results').replace(/\\/g, '/'),
  
  /* Run tests in files in parallel */
  fullyParallel: true,
  /* Disable worker reuse to prevent cache issues */
  workers: 1,
  ...
  reporter: [
    ...
    ['list', { 
      printSteps: true,
      printStdio: 'always'  // 항상 stdout/stderr 출력 (console.log 포함)
    }]
  ],
  /* timeout */
