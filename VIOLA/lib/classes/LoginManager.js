...
class LoginManager extends BaseManager {
  ...
  async processLogin(config) {
    try {
      ...
      console.log('✅ VIOLA 로그인 프로세스 완료');
      return { success: true, message: 'VIOLA 로그인 완료' };
      
    } catch (error) {
      console.error('VIOLA 로그인 실패:', error.message);
      throw error;
    }
  }
  ...
  async navigateToLoginPage() {
    console.log('🌐 VIOLA 로그인 페이지 접근 중...');
    ...
    // 로그인 페이지 접근 스크린샷
    await this.captureScreenshot('VIOLA 로그인 페이지 접근');
    
    console.log('✅ VIOLA 로그인 페이지 접근 완료');
  }
  ...
  async fillLoginCredentials(config) {
    ...
    // 로그인 정보 입력 스크린샷
    await this.captureScreenshot('VIOLA 로그인 정보 입력');
  ...
  async submitLoginAndVerify() {
    ...
    await this.captureScreenshot('VIOLA 로그인 성공');
  ...
  async navigateToDashboard() {
    try {
      console.log('📊 VIOLA 대시보드 접근 중...');
      await this.page.goto('http://viola.test.okestro.cloud/dashboard');
      await this.page.waitForLoadState('networkidle');
      
      // 대시보드 접근 스크린샷
      await this.captureScreenshot('VIOLA 대시보드 접근');
      
      console.log('✅ VIOLA 대시보드 접근 완료');
    } catch (error) {
      console.error('VIOLA 대시보드 접근 실패:', error.message);
      throw error;
    }
  }
  ...
  async logout() {
    try {
      console.log('🚪 VIOLA 로그아웃 중...');
      
      // 로그아웃 버튼 클릭 (실제 구현에 따라 선택자 수정 필요)
      await this.page.getByRole('button', { name: '로그아웃' }).click();
      await this.page.waitForLoadState('networkidle');
      
      // 로그아웃 스크린샷
      await this.captureScreenshot('VIOLA 로그아웃');
      
      console.log('✅ VIOLA 로그아웃 완료');
    } catch (error) {
      console.error('VIOLA 로그아웃 실패:', error.message);
      throw error;
    }
  }
}
