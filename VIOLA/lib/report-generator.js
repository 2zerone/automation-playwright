...
class ReportGenerator {
    ...
    getScenarioDescription(id) {
        try {
            ...
            let settingsPath;
            if (this.currentProduct === 'viola') {
                settingsPath = path.join(__dirname, '..', 'VIOLA', 'config', 'scenario', `test-settings-${id}.json`);
            } else if (this.currentProduct === 'contrabass') {
        ...
        // 제품별 기본 설명 제공
        if (this.currentProduct === 'viola' && id === 1) {
            return 'VIOLA를 통해 생성한 클러스터가 사용가능한지 확인';
        } else if (this.currentProduct === 'viola' && id === 2) {
    ...
    generateTestData(scenarioId, testResults) {
        ...
        const getScenarioName = (id) => {
            try {
                ...
                let scenarioFilePath;
                if (this.currentProduct === 'viola') {
                    scenarioFilePath = path.join(__dirname, '..', 'VIOLA', 'tests', 'scenario', `scenario-${id}.spec.js`);
                } else if (this.currentProduct === 'contrabass') {
                    scenarioFilePath = path.join(__dirname, '..', 'CONTRABASS', 'tests', 'scenario', `scenario-${id}.spec.js`);
                } else if (this.currentProduct === 'cmp') {
                    scenarioFilePath = path.join(__dirname, '..', 'CMP', 'tests', 'scenario', `scenario-${id}.spec.js`);
            ...
            return violaScenarioNames[id] || `시나리오 ${id}`;
        };
    ...
    generateCustomReport(scenarioId, reportData = null) {
        ...
        console.log(`🔍 VIOLA 시나리오 ${scenarioId}: 초기 testResults 상태:`, testResults ? '데이터 있음' : '데이터 없음');
        console.log(`🔍 VIOLA 시나리오 ${scenarioId}: reportData 전체:`, reportData);
        
        // testResults가 null인 경우 실시간 데이터 우선 사용
        if (!testResults) {
            console.log(`⚠️ 시나리오 ${scenarioId}: reportData에서 testResults를 찾을 수 없음. 실시간 데이터 확인 중...`);
            
            // 1. 먼저 VIOLA 전용 global.testResults Map에서 최신 데이터 확인
            ...
            if (global.testResults && global.testResults instanceof Map) {
                ...
                if (latestTestResults) {
                    console.log(`✅ VIOLA 시나리오 ${scenarioId}: global.testResults Map에서 최신 데이터 발견`);
                    console.log(`📊 VIOLA 시나리오 ${scenarioId}: Map 데이터 상태: ${latestTestResults.status}, 테스트 케이스: ${latestTestResults.testCases?.length || 0}개`);
                    testResults = latestTestResults;
                    reportData = reportData || {};
                    reportData.testResults = testResults;
                }
            } else {
                console.log(`⚠️ VIOLA 시나리오 ${scenarioId}: global.testResults Map이 존재하지 않음`);
            }
                ...
                try {
                    ...
                    if (fs.existsSync(globalTestResultsPath)) {
                        ...
                        if (testResultsData[scenarioKey]) {
                            ...
                            console.log(`✅ VIOLA 시나리오 ${scenarioId}: global-test-results.json에서 데이터 발견`);
                            
                            // testResults를 reportData에 설정
                            reportData = reportData || {};
                            reportData.testResults = testResults;
                        } else {
                            console.log(`⚠️ VIOLA 시나리오 ${scenarioId}: global-test-results.json에서 VIOLA 전용 데이터를 찾을 수 없음`);
                        }
                    } else {
            ...
            console.log(`✅ VIOLA 시나리오 ${scenarioId}: reportData에서 testResults 발견`);
            console.log(`📊 VIOLA 시나리오 ${scenarioId}: testResults 상태: ${testResults.status}, 테스트 케이스: ${testResults.testCases?.length || 0}개`);
        }
        
        // 강제로 실시간 데이터 우선 사용 (VIOLA 전용) - 캐시 무시
        if (global.testResults && global.testResults instanceof Map) {
            const scenarioKey = `viola-scenario-${scenarioId}`;
            const latestTestResults = global.testResults.get(scenarioKey);
            if (latestTestResults) {
                console.log(`🔄 VIOLA 시나리오 ${scenarioId}: 캐시 무시하고 최신 데이터 강제 사용`);
                console.log(`📊 VIOLA 시나리오 ${scenarioId}: Map 최신 데이터 - 상태: ${latestTestResults.status}, 테스트 케이스: ${latestTestResults.testCases?.length || 0}개`);
                ...
                console.log(`✅ VIOLA 캐시 무시 완료: 새로운 타임스탬프 ${testResults.timestamp}`);
            }
        }
        ...
        // 우선순위: 3. testResults에서 전달된 데이터 사용 (VIOLA 동적 파싱 결과 우선 처리)
        if (testCases.length === 0 && testResults) {
            if (testResults.testCases && testResults.testCases.length > 0) {
                ...
                console.log(`📊 VIOLA 동적 파싱 결과 사용 - 빈 테스트 케이스 처리`);
                // 빈 테스트 케이스로 처리 (실제 결과 반영)
                testCases = [];
                scenarioStatus = testResults.status || 'fail';
                scenarioDuration = testResults.duration || 'N/A';
                console.log(`📊 VIOLA 동적 파싱 결과로 커스텀 리포트 생성 완료`);
        ...
        console.log(`📊 VIOLA 실시간 데이터 우선 사용 (파일 캐시 완전 무시)`);
        ...
        if (testResults && testResults.testCases !== undefined) {
            console.log(`✅ VIOLA 시나리오 ${scenarioId}: 실시간 testResults에서 데이터 발견`);
            console.log(`📊 VIOLA 실시간 상태: ${testResults.status}`);
            console.log(`📊 VIOLA 실시간 테스트 케이스 수: ${testResults.testCases.length}개`);
            ...
            console.log(`📊 VIOLA 실시간 데이터 사용 완료: ${testCases.length}개 테스트, 상태: ${scenarioStatus}`);
            console.log(`📊 VIOLA testCases 상세: ${testCases.map((tc, i) => `[${i}] ${tc.name}: ${tc.status}`).join(', ')}`);
            dynamicResultFound = true;
        } else {
            console.log(`⚠️ VIOLA 시나리오 ${scenarioId}: 실시간 testResults에서 데이터를 찾을 수 없음`);
            ...
            try {
                ...
                if (fs.existsSync(globalResultsPath)) {
                    ...
                    if (globalResults[scenarioKey]) {
                        console.log(`📊 VIOLA 파일에서 시나리오 ${scenarioId} 결과 발견 (fallback)`);
                        ...
                        console.log(`📊 VIOLA 파일 데이터 사용 (fallback): ${testCases.length}개 테스트, 상태: ${scenarioStatus}`);
                        dynamicResultFound = true;
                    }
                }
            } catch (error) {
                console.log(`❌ VIOLA 파일 읽기 실패: ${error.message}`);
            }
        }
        
        // 동적 결과를 찾았으면 다른 fallback 로직 건너뛰기
        if (dynamicResultFound) {
            console.log(`📊 VIOLA 동적 결과 사용 - fallback 로직 완전히 건너뛰기`);
        ...
        if (testCases.length === 0 && !dynamicResultFound) {
            ...
            if (testResults && testResults.testCases && testResults.testCases.length === 0) {
                ...
                console.log(`📊 VIOLA fallback 로직 건너뛰기 완료`);
...
<head>
    ...
    <title>VIOLA 자동화 테스트 커스텀 리포트 - ${currentScenarioName}</title>
...
<body>
    <div class="container">
        ...
        <div class="content">
            <h2 class="section-title">클러스터 정보</h2>
            <div class="project-info">
                <div class="project-header">
                    <h3><i class="fas fa-project-diagram"></i> 클러스터명 - biz-cluster-1</h3>
                </div>
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>VIOLA 자동화 테스트 마스터 리포트</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <style>
        * { 
            margin: 0; 
...
<body>
    <div class="container">
        <div class="header">
            <h1><i class="fas fa-music"></i> VIOLA 자동화 테스트</h1>
    ...
    saveMasterReport() {
        try {
            ...
            let customReportsPath;
            if (this.currentProduct === 'viola') {
                customReportsPath = path.join(__dirname, '..', 'VIOLA', 'custom-reports');
            } else if (this.currentProduct === 'contrabass') {
                customReportsPath = path.join(__dirname, '..', 'CONTRABASS', 'custom-reports');
            } else if (this.currentProduct === 'cmp') {
                customReportsPath = path.join(__dirname, '..', 'CMP', 'custom-reports');
    ...
    mapScreenshotsToTestCases(testCases, scenarioId) {
        try {
            ...
            testCases.forEach(testCase => {
                ...
                // VIOLA/custom-reports/scenario-{scenarioId} 디렉토리에서 해당 테스트 케이스의 스크린샷 찾기
                const scenarioDir = path.join(__dirname, '..', 'custom-reports', `scenario-${scenarioId}`);
                const screenshots = [];
                ...
                const matchingScreenshots = screenshots.filter(screenshot => {
                    ...
                    // 기존 VIOLA 스타일: {stepName}_{status}_{timestamp}.png
                    const cleanTestCaseName = testCaseName.replace(/[^가-힣a-zA-Z0-9]/g, '');
