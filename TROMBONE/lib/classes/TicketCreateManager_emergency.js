...
class TicketCreateManagerEmergency extends BaseManager {
  constructor(utils) {
    super(utils);
  }
  ...
  async configureWorkflowAndApproval() {
    try {
      // 긴급 워크플로우 선택
      await this.utils.page.getByText('긴급', { exact: true }).click();
      await this.utils.page.getByRole('cell', { name: '긴급' }).getByPlaceholder('선택').click();
      await this.utils.page.getByRole('cell', { name: '긴급' }).getByPlaceholder('선택').fill('HOTF-긴급 워크플로우');
...
export default TicketCreateManagerEmergency;