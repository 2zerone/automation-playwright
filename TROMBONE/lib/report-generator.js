...
class ReportGenerator {
    ...
    cleanStepName(stepName) {
        ...
        // "중메뉴", "대메뉴", ": " 접두사 등의 문구 제거 (단, "메뉴 접근" 같은 실제 단계명은 유지)
        let cleanedName = stepName
            .replace(/^:\s*/, '')  // ": " 접두사 제거
            .replace(/중메뉴\s*/g, '')
            .replace(/대메뉴\s*/g, '')
            // "메뉴" 단어 자체를 제거하지 않음 (실제 단계명에 포함되어 있음)
            .trim();
    ...
    loadTestSettings() {
        try {
            ...
            return {
                ...
                project: { code: "TROMBONE", name: "kubernetes" },
                repository: { name: "TROMBONE-kubernetes" },
                users: [],
                login: { userId: '' }
            };
        }
    ...
    // 시나리오 파일에서 allPlannedTestCases 읽기
    getAllPlannedTestCases(scenarioId) {
        try {
            let scenarioFilePath;
            if (this.currentProduct === 'trombone') {
                scenarioFilePath = path.join(__dirname, '..', 'tests', 'scenario', `scenario-${scenarioId}.spec.js`);
            } else if (this.currentProduct === 'contrabass') {
                scenarioFilePath = path.join(__dirname, '..', 'CONTRABASS', 'tests', 'scenario', `scenario-${scenarioId}.spec.js`);
            } else if (this.currentProduct === 'cmp') {
                scenarioFilePath = path.join(__dirname, '..', 'CMP', 'tests', 'scenario', `scenario-${scenarioId}.spec.js`);
            } else {
                scenarioFilePath = path.join(__dirname, '..', 'tests', 'scenario', `scenario-${scenarioId}.spec.js`);
            }
            ...
            if (fs.existsSync(scenarioFilePath)) {
                ...
                // allPlannedTestCases 배열 추출 (더 정확한 패턴)
                const match = content.match(/const allPlannedTestCases = \[([\s\S]*?)\];/);
                if (match) {
                    // 정확한 파싱: { name: '...', status: '...' } 패턴 추출
                    const testCases = [];
                    // 여러 줄에 걸친 name 필드도 처리할 수 있도록 개선
                    const namePattern = /\{\s*name:\s*['"]([^'"]+)['"]/g;
                    let nameMatch;
                    while ((nameMatch = namePattern.exec(match[1])) !== null) {
                        testCases.push(nameMatch[1]);
                    }
                    console.log(`📋 allPlannedTestCases 읽기 완료: ${testCases.length}개 단계`);
                    return testCases;
                } else {
                    console.warn(`⚠️ allPlannedTestCases 배열을 찾을 수 없습니다: ${scenarioFilePath}`);
                }
            } else {
                console.warn(`⚠️ 시나리오 파일을 찾을 수 없습니다: ${scenarioFilePath}`);
            }
        } catch (error) {
            console.error(`❌ allPlannedTestCases 읽기 실패: ${error.message}`);
        }
        return [];
    }
    ...
    getScenarioDescription(id) {
        try {
            ...
            let settingsPath;
            if (this.currentProduct === 'trombone') {
                settingsPath = path.join(__dirname, '..', 'TROMBONE', 'config', 'scenario', `test-settings-${id}.json`);
            } else if (this.currentProduct === 'contrabass') {
        ...
        // 제품별 기본 설명 제공
        if (this.currentProduct === 'trombone' && id === 1) {
            return 'TROMBONE를 통해 생성한 클러스터가 사용가능한지 확인';
        } else if (this.currentProduct === 'trombone' && id === 2) {
    ...
    generateTestData(scenarioId, testResults) {
        ...
        const getScenarioName = (id) => {
            try {
                ...
                let scenarioFilePath;
                if (this.currentProduct === 'trombone') {
                    scenarioFilePath = path.join(__dirname, '..', 'TROMBONE', 'tests', 'scenario', `scenario-${id}.spec.js`);
                } else if (this.currentProduct === 'contrabass') {
                    scenarioFilePath = path.join(__dirname, '..', 'CONTRABASS', 'tests', 'scenario', `scenario-${id}.spec.js`);
                } else if (this.currentProduct === 'cmp') {
                    scenarioFilePath = path.join(__dirname, '..', 'CMP', 'tests', 'scenario', `scenario-${id}.spec.js`);
            ...
            // TROMBONE 기본값 반환
            const tromboneScenarioNames = {
                1: 'TROMBONE 클러스터 생성 및 확인',
                2: '외부접속 서비스 트래픽 모니터링',
            ...
            return tromboneScenarioNames[id] || `시나리오 ${id}`;
        };
        ...
        // 시나리오별 테스트 케이스 정의 - 시나리오 파일에서 추출하도록 변경
        const getScenarioTestCases = (id) => {
            // 시나리오 파일에서 실제 테스트 케이스 추출 (다른 제품들과 동일한 방식)
            const extractedCases = this.extractTestCasesFromScenarioFile(id, 'pass');
            if (extractedCases && extractedCases.length > 0) {
                // 추출된 테스트 케이스를 description 포함 형식으로 변환
                return extractedCases.map(testCase => ({
                    name: testCase.name,
                    description: testCase.name // name을 description으로도 사용
                }));
            }
            
            // 추출 실패 시 빈 배열 반환 (하드코딩된 목록 사용하지 않음)
            return [];
        };
    ...
    generateCustomReport(scenarioId, reportData = null) {
        ...
        console.log(`🔍 TROMBONE 시나리오 ${scenarioId}: 초기 testResults 상태:`, testResults ? '데이터 있음' : '데이터 없음');
        console.log(`🔍 TROMBONE 시나리오 ${scenarioId}: reportData 전체:`, reportData);
        
        // testResults가 null인 경우 실시간 데이터 우선 사용
        if (!testResults) {
            console.log(`⚠️ 시나리오 ${scenarioId}: reportData에서 testResults를 찾을 수 없음. 실시간 데이터 확인 중...`);
            
            // 1. 먼저 TROMBONE 전용 global.testResults Map에서 최신 데이터 확인
            let latestTestResults = null;
            if (global.testResults && global.testResults instanceof Map) {
                const scenarioKey = `trombone-scenario-${scenarioId}`;
                latestTestResults = global.testResults.get(scenarioKey);
                if (latestTestResults) {
                    console.log(`✅ TROMBONE 시나리오 ${scenarioId}: global.testResults Map에서 최신 데이터 발견`);
                    console.log(`📊 TROMBONE 시나리오 ${scenarioId}: Map 데이터 상태: ${latestTestResults.status}, 테스트 케이스: ${latestTestResults.testCases?.length || 0}개`);
                    testResults = latestTestResults;
                    reportData = reportData || {};
                    reportData.testResults = testResults;
                }
            } else {
                console.log(`⚠️ TROMBONE 시나리오 ${scenarioId}: global.testResults Map이 존재하지 않음`);
            }
            ...
            if (!testResults) {
                try {
                    ...
                    if (fs.existsSync(globalTestResultsPath)) {
                        ...
                        // TROMBONE 전용 키로 데이터 찾기
                        const scenarioKey = `trombone-scenario-${scenarioId}`;
                        if (testResultsData[scenarioKey]) {
                            testResults = testResultsData[scenarioKey];
                            console.log(`✅ TROMBONE 시나리오 ${scenarioId}: global-test-results.json에서 데이터 발견`);
                            
                            // testResults를 reportData에 설정
                            reportData = reportData || {};
                            reportData.testResults = testResults;
                        } else {
                            console.log(`⚠️ TROMBONE 시나리오 ${scenarioId}: global-test-results.json에서 TROMBONE 전용 데이터를 찾을 수 없음`);
                        }
                    } else {
            ...
            console.log(`✅ TROMBONE 시나리오 ${scenarioId}: reportData에서 testResults 발견`);
            console.log(`📊 TROMBONE 시나리오 ${scenarioId}: testResults 상태: ${testResults.status}, 테스트 케이스: ${testResults.testCases?.length || 0}개`);
        }
        
        // 강제로 실시간 데이터 우선 사용 (TROMBONE 전용) - 캐시 무시
        if (global.testResults && global.testResults instanceof Map) {
            const scenarioKey = `trombone-scenario-${scenarioId}`;
            const latestTestResults = global.testResults.get(scenarioKey);
            if (latestTestResults) {
                console.log(`🔄 TROMBONE 시나리오 ${scenarioId}: 캐시 무시하고 최신 데이터 강제 사용`);
                console.log(`📊 TROMBONE 시나리오 ${scenarioId}: Map 최신 데이터 - 상태: ${latestTestResults.status}, 테스트 케이스: ${latestTestResults.testCases?.length || 0}개`);
                ...
                console.log(`✅ TROMBONE 캐시 무시 완료: 새로운 타임스탬프 ${testResults.timestamp}`);
            }
        }
        ...
        // 우선순위: 3. testResults에서 전달된 데이터 사용 (TROMBONE 동적 파싱 결과 우선 처리)
        if (testCases.length === 0 && testResults) {
            if (testResults.testCases && testResults.testCases.length > 0) {
                ...
                console.log(`📊 TROMBONE 동적 파싱 결과 사용 - 빈 테스트 케이스 처리`);
                // 빈 테스트 케이스로 처리 (실제 결과 반영)
                testCases = [];
                scenarioStatus = testResults.status || 'fail';
                scenarioDuration = testResults.duration || 'N/A';
                console.log(`📊 TROMBONE 동적 파싱 결과로 커스텀 리포트 생성 완료`);
        ...
        console.log(`📊 TROMBONE 실시간 데이터 우선 사용 (파일 캐시 완전 무시)`);
        ...
        if (testResults && testResults.testCases !== undefined) {
            console.log(`✅ TROMBONE 시나리오 ${scenarioId}: 실시간 testResults에서 데이터 발견`);
            console.log(`📊 TROMBONE 실시간 상태: ${testResults.status}`);
            console.log(`📊 TROMBONE 실시간 테스트 케이스 수: ${testResults.testCases.length}개`);
            ...
            console.log(`📊 TROMBONE 실시간 데이터 사용 완료: ${testCases.length}개 테스트, 상태: ${scenarioStatus}`);
            console.log(`📊 TROMBONE testCases 상세: ${testCases.map((tc, i) => `[${i}] ${tc.name}: ${tc.status}, duration=${tc.duration}ms`).join(', ')}`);
            // 디버깅: 처음 5개 testCase의 상세 정보 출력
            testCases.slice(0, 5).forEach((tc, i) => {
                console.log(`🔍 [testCase ${i}] ${tc.name}: status=${tc.status}, duration=${tc.duration}, hasResults=${tc.hasResults}`);
            });
            dynamicResultFound = true;
        } else {
            console.log(`⚠️ TROMBONE 시나리오 ${scenarioId}: 실시간 testResults에서 데이터를 찾을 수 없음`);
            ...
            try {
                ...
                if (fs.existsSync(globalResultsPath)) {
                    ...
                    if (globalResults[scenarioKey]) {
                        console.log(`📊 TROMBONE 파일에서 시나리오 ${scenarioId} 결과 발견 (fallback)`);
                        ...
                        console.log(`📊 TROMBONE 파일 데이터 사용 (fallback): ${testCases.length}개 테스트, 상태: ${scenarioStatus}`);
                        dynamicResultFound = true;
                    }
                }
            } catch (error) {
                console.log(`❌ TROMBONE 파일 읽기 실패: ${error.message}`);
            }
        }
        
        // 동적 결과를 찾았으면 다른 fallback 로직 건너뛰기
        if (dynamicResultFound) {
            console.log(`📊 TROMBONE 동적 결과 사용 - fallback 로직 완전히 건너뛰기`);
        ...
        if (testCases.length === 0 && !dynamicResultFound) {
            ...
            if (testResults && testResults.testCases && testResults.testCases.length === 0) {
                ...
                console.log(`📊 TROMBONE fallback 로직 건너뛰기 완료`);
...
<head>
    ...
    <title>TROMBONE 자동화 테스트 커스텀 리포트 - ${currentScenarioName}</title>
...
<body>
    <div class="container">
        ...
        <div class="content">
            <h2 class="section-title">테스트 환경 정보</h2>
            <div class="project-info">
                <div class="project-header">
                    <h3><i class="fas fa-project-diagram"></i>검증 환경-V3.0.5</h3>
                </div>
                    ...
                    <tbody id="test-results-body">
                        ${uniqueTestCases
                            ...
                            if (testCase.status === 'pass') {
                                ...
                                stepStatusText = '미수행';
                            } else if (testCase.status === 'not-test' || testCase.status === 'not-executed') {
                                stepStatusClass = 'not-test';
                                stepStatusText = '미수행';
                            }
                            ...
                            if (testCase.duration) {
                                // 디버깅: duration 값 확인 (처음 5개만)
                                if (index < 5) {
                                    console.log(`🔍 [리포트 생성] ${testCase.name}: testCase.duration=${testCase.duration} (타입: ${typeof testCase.duration})`);
                                }
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>TROMBONE 자동화 테스트 마스터 리포트</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <style>
        * { 
            margin: 0; 
...
<body>
    <div class="container">
        <div class="header">
            <h1><i class="fas fa-music"></i> TROMBONE 자동화 테스트</h1>
    ...
    saveMasterReport() {
        try {
            ...
            let customReportsPath;
            if (this.currentProduct === 'trombone') {
                customReportsPath = path.join(__dirname, '..', 'TROMBONE', 'custom-reports');
            } else if (this.currentProduct === 'contrabass') {
                customReportsPath = path.join(__dirname, '..', 'CONTRABASS', 'custom-reports');
            } else if (this.currentProduct === 'cmp') {
                customReportsPath = path.join(__dirname, '..', 'CMP', 'custom-reports');
        ...
        try {
            ...
            if (currentScenario) {
                ...
                if (testResults) {
                    ...
                    const passedSteps = uniqueTestCases.filter(step => step.status === 'pass').length;
                    const failedSteps = uniqueTestCases.filter(step => step.status === 'fail' || step.status === 'stopped').length;
                    const notTestSteps = uniqueTestCases.filter(step => step.status === 'not-test' || step.status === 'not-executed').length;
                    const executedSteps = totalSteps - notTestSteps;
                    
                    // 실제 테스트 결과를 우선 사용 (상태 통일: pass)
                    let finalStatus = testResults.status || 'pass';
                    
                    // 디버깅: 전달받은 status 확인
                    console.log(`🔍 [updateScenarioList] 시나리오 ${scenarioId} - testResults.status: "${testResults.status}"`);
                    console.log(`🔍 [updateScenarioList] failedSteps: ${failedSteps}, passedSteps: ${passedSteps}, executedSteps: ${executedSteps}`);
                    
                    // testResults.status가 명시적으로 설정된 경우 상태 통일
                    if (testResults.status) {
                        ...
                        console.log(`✅ [updateScenarioList] testResults.status 변환: "${testResults.status}" → "${finalStatus}"`);
                    } else {
                        ...
                        console.log(`⚠️ [updateScenarioList] 테스트 케이스 기반 계산: "${finalStatus}"`);
                    }
    ...
    generateScreenshotsHtml(testCase, scenarioId = 1) {
        ...
        if (testCase.status === 'not-test' || testCase.status === 'not-executed' || testCase.status === 'pending') {
            return `
    ...
    mapScreenshotsToTestCases(testCases, scenarioId) {
        try {
            ...
            testCases.forEach(testCase => {
                ...
                // TROMBONE/custom-reports/scenario-{scenarioId} 디렉토리에서 해당 테스트 케이스의 스크린샷 찾기
                const scenarioDir = path.join(__dirname, '..', 'custom-reports', `scenario-${scenarioId}`);
                const screenshots = [];
                ...
                const matchingScreenshots = screenshots.filter(screenshot => {
                    ...
                    // 기존 TROMBONE 스타일: {stepName}_{status}_{timestamp}.png
                    const cleanTestCaseName = testCaseName.replace(/[^가-힣a-zA-Z0-9]/g, '');
