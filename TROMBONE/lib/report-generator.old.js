...
class ReportGenerator {
    ...
    generateTestDataFromResults(scenarioId, testResults) {
        ...
        const maxScenarios = scenarioCounts[this.currentProduct] || 21;
    ...
    generateCustomReport(scenarioId, reportData = null) {
        ...
        // 실제 데이터 사용 (설정 파일 우선, reportData 다음, 기본값 마지막)
        const testResults = reportData?.testResults || null;
        const user = currentConfig?.user || reportData?.user || { id: 'yh.lee5', name: '이영한' };
        const project = currentConfig?.project || reportData?.project || { code: "LYH", name: "LYH 업무코드" };
        ...
        console.log(`📊 [DEBUG] 시나리오 ${scenarioId} 데이터 소스 확인 시작`);
        if (fs.existsSync(globalResultsPath)) {
            try {
                const globalResults = JSON.parse(fs.readFileSync(globalResultsPath, 'utf8'));
                console.log(`📊 [DEBUG] global-test-results.json 키들:`, Object.keys(globalResults));
                const scenarioResults = globalResults[scenarioId] || globalResults[`scenario-${scenarioId}`];
                console.log(`📊 [DEBUG] 시나리오 ${scenarioId} 결과 찾기: scenarioId=${scenarioId}, scenario-${scenarioId}=${globalResults[`scenario-${scenarioId}`] ? 'found' : 'not found'}`);
                console.log(`📊 [DEBUG] 시나리오 ${scenarioId} 결과:`, scenarioResults ? `${scenarioResults.testCases?.length || 0}개 테스트 케이스` : 'not found');
                
                if (scenarioResults && scenarioResults.testCases && scenarioResults.testCases.length > 0) {
                    console.log(`📊 [DEBUG] 시나리오 ${scenarioId} 데이터 발견: ${scenarioResults.testCases.length}개 테스트 케이스`);
        ...
        // 우선순위: 3. testResults에서 전달된 데이터 사용
        if (testCases.length === 0 && testResults && testResults.testCases && testResults.testCases.length > 0) {
        ...
        console.log(`🔍 [DURATION DEBUG] scenarioDuration: ${scenarioDuration}, duration: ${duration}, testResults.duration: ${testResults?.duration}`);
        ...
        <div class="content">
            ...
            <div class="project-info">
                <div class="project-header">
                    <h3><i class="fas fa-project-diagram"></i> ${project.code} - ${project.name}</h3>
                </div>
                <div class="project-details">
                    ...
                    <div class="detail-item">
                        ...
                        <div class="detail-value">${repository.name}</div>
                    </div>
                    <div class="detail-item">
                        <div class="detail-label">테스트 환경</div>
                        <div class="detail-value">검증 환경</div>
                    </div>
                </div>
            </div>
                ...
                <table id="test-results-table">
                    ...
                    <tbody id="test-results-body">
                        ${uniqueTestCases
                            ...
                            // 실패 로그 추가 (전역 실패 로그에서 해당 테스트 케이스와 관련된 로그 찾기)
                            ...
                            // 실패 로그가 있으면 defaultLogs에 추가 (상세한 에러 메시지)
...
<body>
    ...
    saveMasterReport() {
        try {
            ...
            const masterReportPath = path.join(customReportsPath, 'test_results_master.html');
            ...
            <div class="scenario-history">
                ...
                <ul class="history-list">
                        ${history.map((item, index) => {
                            ...
                            // 가장 최근 히스토리(첫 번째 항목)만 global-test-results.json에서 상태 가져오기
                            if (index === 0) {
                                try {
                                    const globalResultsPath = path.join(__dirname, '..', 'custom-reports', 'global-test-results.json');
                                    if (fs.existsSync(globalResultsPath)) {
    ...
    updateScenarioList(scenarioId, testResults = null) {
        try {
            ...
            const scenarioListPath = path.join(customReportsPath, 'scenario-list.json');
            ...
            if (currentScenario) {
                ...
                if (testResults) {
                    ...
                    if (finalStatus === 'stopped' || finalStatus === 'fail') {
                        // 이미 실패 상태로 설정된 경우 그대로 유지
                    } else if (failedSteps > 0) {
                        finalStatus = 'fail';
            ...
            // 마스터 리포트도 즉시 업데이트 (상태 보존을 위해 별도 처리)
    ...
    mapScreenshotsToTestCases(testCases, scenarioId) {
        try {
            ...
            testCases.forEach(testCase => {
                ...
                // custom-reports/scenario-{scenarioId} 디렉토리에서 해당 테스트 케이스의 스크린샷 찾기
                const scenarioDir = path.join(__dirname, '..', 'custom-reports', `scenario-${scenarioId}`);
                const screenshots = [];
                ...
                // 스크린샷 파일명에서 테스트 케이스 이름과 매칭되는 파일 찾기
                const matchingScreenshots = screenshots.filter(screenshot => {
                    // 스크린샷 파일명에서 테스트 케이스 이름이 포함되어 있는지 확인
                ...
                if (matchingScreenshots.length > 0) {
                    ...
                    const latestScreenshot = matchingScreenshots.sort().pop(); // 가장 최신 파일
