...
const allPlannedTestCases = [
  ...
  // 13. 코드리뷰 단계 세부 단계 (1~17단계를 논리적으로 그룹화)
  { name: '코드리뷰 단계: 코드리뷰 접근', status: 'pending' },        // 1~2단계
  { name: '코드리뷰 단계: 검토 의견 입력', status: 'pending' },        // 3~7단계
  { name: '코드리뷰 단계: 테스트 케이스 관리 진입', status: 'pending' },    // 8단계
  { name: '코드리뷰 단계: 테스트 케이스 등록 화면 진입', status: 'pending' }, // 9단계
  { name: '코드리뷰 단계: 테스트 케이스 정보 입력', status: 'pending' },     // 10~12단계
  { name: '코드리뷰 단계: 테스트 케이스 추가', status: 'pending' },     // 13~14단계
  { name: '코드리뷰 단계: 코드 검토', status: 'pending' },             // 15~16단계
  { name: '코드리뷰 단계: 코드리뷰 단계 완료 및 다음 단계 진입', status: 'pending' },           // 17단계
  ...
  // 15. 단위테스트 단계 세부 단계
  { name: '단위테스트 단계: 단위테스트 진행', status: 'pending' },             // 단위테스트 성공 확인
  { name: '단위테스트 단계: 단위테스트 단계 완료 및 다음 단계 진입', status: 'pending' }, // 다음 단계 진행
  
  // 16. 정적분석 단계 세부 단계
  { name: '정적분석 단계: 정적분석 진행', status: 'pending' },                 // 정적분석 성공 확인
  { name: '정적분석 단계: 정적분석 단계 완료 및 다음 단계 진입', status: 'pending' }, // 다음 단계 진행
...
// 시나리오 1: CI/CD 일반 티켓을 통한 K8S 배포 시나리오 (예약 배포 / 직렬 결재)
test.describe.serial('시나리오 1: CI/CD 일반 티켓을 통한 K8S 배포 시나리오', () => {
  let page;
  let browser;
  let config;
  let managers;
  ...
  test.beforeAll(async ({ browser: browserInstance }) => {
    ...
    // 테스트 시작을 명확하게 표시
    console.log('🚀 시나리오 1 테스트 시작');
    process.stdout.write('🚀 시나리오 1 테스트 시작\n');
  ...
  test('코드리뷰 단계: 코드리뷰 접근', async () => {
    await runTestStep('코드리뷰 단계: 코드리뷰 접근', async () => {
      await managers.ticketCodeReviewManager.accessCodeReview();
    }, page);
  });

  test('코드리뷰 단계: 검토 의견 입력', async () => {
    await runTestStep('코드리뷰 단계: 검토 의견 입력', async () => {
      await managers.ticketCodeReviewManager.inputReviewComment(config);
    }, page);
  });

  test('코드리뷰 단계: 테스트 케이스 관리 진입', async () => {
    await runTestStep('코드리뷰 단계: 테스트 케이스 관리 진입', async () => {
      await managers.ticketCodeReviewManager.enterTestCaseManagement();
    }, page);
  });

  test('코드리뷰 단계: 테스트 케이스 등록 화면 진입', async () => {
    await runTestStep('코드리뷰 단계: 테스트 케이스 등록 화면 진입', async () => {
      await managers.ticketCodeReviewManager.enterTestCaseRegistration();
    }, page);
  });

  test('코드리뷰 단계: 테스트 케이스 정보 입력', async () => {
    await runTestStep('코드리뷰 단계: 테스트 케이스 정보 입력', async () => {
      await managers.ticketCodeReviewManager.fillTestCaseInfo(config);
    }, page);
  });

  test('코드리뷰 단계: 테스트 케이스 추가', async () => {
    await runTestStep('코드리뷰 단계: 테스트 케이스 추가', async () => {
      await managers.ticketCodeReviewManager.addTestCase();
    }, page);
  });

  test('코드리뷰 단계: 코드 검토', async () => {
    await runTestStep('코드리뷰 단계: 코드 검토', async () => {
      await managers.ticketCodeReviewManager.reviewCode();
    }, page);
  });

  test('코드리뷰 단계: 코드리뷰 단계 완료 및 다음 단계 진입', async () => {
    await runTestStep('코드리뷰 단계: 코드리뷰 단계 완료 및 다음 단계 진입', async () => {
      await managers.ticketCodeReviewManager.completeAndProceedToNext();
    }, page);
  });
  ...
  test('단위테스트 단계: 단위테스트 진행', async () => {
    await runTestStep('단위테스트 단계: 단위테스트 진행', async () => {
      await managers.ticketUnitTestManager.proceedUnitTest();
    }, page);
  });

  test('단위테스트 단계: 단위테스트 단계 완료 및 다음 단계 진입', async () => {
    await runTestStep('단위테스트 단계: 단위테스트 단계 완료 및 다음 단계 진입', async () => {
      await managers.ticketUnitTestManager.completeAndProceedToNext();
    }, page);
  });

  test('정적분석 단계: 정적분석 진행', async () => {
    await runTestStep('정적분석 단계: 정적분석 진행', async () => {
      await managers.ticketStaticAnalysisManager.proceedStaticAnalysis();
    }, page);
  });

  test('정적분석 단계: 정적분석 단계 완료 및 다음 단계 진입', async () => {
    await runTestStep('정적분석 단계: 정적분석 단계 완료 및 다음 단계 진입', async () => {
      await managers.ticketStaticAnalysisManager.completeAndProceedToNext();
    }, page);
  });
