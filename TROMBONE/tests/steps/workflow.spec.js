...
// 대메뉴: 워크플로우 컴포넌트 관리
test.describe.serial('워크플로우 컴포넌트 관리', () => {
  
  // 중메뉴: 워크플로우 컴포넌트 등록
  test.describe.serial('워크플로우 컴포넌트 등록', () => {
    test('상세결과', async () => {
      console.log('⚙️ 워크플로우 컴포넌트 등록 및 검증 시작...');
    ...
    const components = [
      ...
      { name: `${global.config.project.code}-운영배포(예약배포)PRD`, buttonIndex: 10, environment: 'PRD', envRowIndex: 3 }
    ];
    ...
    console.log('✅ 워크플로우 컴포넌트 등록 및 검증 완료\n');
    });
  });
}); 