...
console.log('📊 CMP Electron 시나리오 실행기\n');

// 공통 함수: global-test-results.json 저장 (VIOLA 방식)
function saveGlobalTestResults(scenarioId, testResultsData, logMessage) {
    console.log(`\n🔍 [CMP DEBUG] saveGlobalTestResults 호출됨!`);
    console.log(`🔍 [CMP DEBUG] scenarioId: ${scenarioId}, 타입: ${typeof scenarioId}`);
    console.log(`🔍 [CMP DEBUG] testResultsData.testCases?.length: ${testResultsData.testCases?.length}`);
    ...
    if (fs.existsSync(globalTestResultsPath)) {
        try {
            ...
            console.log(`🔍 [CMP DEBUG] 기존 키 개수: ${Object.keys(globalTestResultsData).length}`);
        } catch (error) {
    ...
    console.log(`🔍 [CMP DEBUG] 저장할 키: ${key}`);
    globalTestResultsData[key] = testResultsData;
    console.log(`🔍 [CMP DEBUG] 저장 후 키 개수: ${Object.keys(globalTestResultsData).length}`);
...
// CMP 시나리오 구조 레포트 출력 함수
function printCmpReport(scenarioId) {
    console.log(`\n📊 CMP 시나리오 ${scenarioId} 구조 레포트`);
    console.log('=' .repeat(60));
    
    // CMP 시나리오 목록
    const cmpScenarios = [
        'CMP 기본 로그인 및 대시보드 확인',
        'CMP 워크플로우 생성 및 관리',
        'CMP 티켓 생성 및 승인 프로세스',
        'CMP GitLab 연동 테스트',
        'CMP 파이프라인 실행 및 모니터링',
    ...
    cmpScenarios.forEach((scenario, index) => {
...
async function runScenario(scenarioId) {
    ...
    try {
        console.log(`🎵 CMP 시나리오 ${scenarioId} 실행을 시작합니다...\n`);
        console.log(`⏰ 시작 시간: ${startTime.toLocaleString('ko-KR')}`);
        
        // CMP 시나리오 구조 레포트 출력
        printCmpReport(scenarioId);
        ...
        try {
            await new Promise((resolve, reject) => {
                ...
                // 실시간 로그 출력 - 터미널에 표시 (중복 체크 제거하여 TROMBONE/main.js로 전달)
            ...
            console.log('✅ CMP 시나리오 실행 완료');
            
        } catch (error) {
            console.log(`❌ CMP 시나리오 실행 실패: ${error.message}`);
            throw error;
        }
        ...
        console.log('\n============================================================');
        console.log(`${testExitCode === 0 ? '✅' : '⚠️'} CMP 시나리오 실행 완료`);
        ...
        console.log('\n📊 새로운 커스텀 리포트 생성 중...');
        try {
            // CMP의 report-generator.js를 직접 호출하여 커스텀 리포트 생성
            ...
            console.log(`📊 [CMP] test-results.json 파일 확인: ${testResultsPath}`);
            
            if (fs.existsSync(testResultsPath)) {
                try {
                    console.log(`📊 [CMP] Playwright 결과 파일 발견`);
                    const testResultsData = JSON.parse(fs.readFileSync(testResultsPath, 'utf8'));
                    
                    console.log(`🔍 [CMP DEBUG] testExitCode: ${testExitCode}`);
                    console.log(`🔍 [CMP DEBUG] test-results.json suites 수: ${testResultsData.suites ? testResultsData.suites.length : 0}`);
                    ...
                    if (scenarioSuite) {
                        console.log(`✅ [CMP] 시나리오 ${scenarioId} 테스트 결과 발견`);
                        ...
                        console.log(`📊 [CMP] 발견된 테스트 스펙: ${allSpecs.length}개`);
                        ...
                        console.log(`📊 [CMP] 전체 테스트: ${allTests.length}개, results 있음: ${allTests.filter(t => t.hasResults).length}개`);
                        ...
                        console.log(`📊 [CMP] 파싱된 테스트 케이스: ${testCases.length}개, 파싱 상태: ${parsedResults.status}`);
                        
                        // 브라우저 강제 종료 처리 (VIOLA 방식 - testExitCode와 관계없이 항상 체크)
                        if (testCases.length > 0) {
                            console.log(`📊 [CMP] 브라우저 강제 종료 지점 탐색 시작 (총 ${testCases.length}개 테스트)`);
                            console.log(`📊 [CMP] testCases 상세:`, testCases.map((t, i) => `[${i}] ${t.name}: ${t.status} (hasResults: ${t.hasResults})`).join(', '));
                            
                            // results가 없는 테스트가 있는지 확인 (VIOLA 방식)
                            const hasTestsWithoutResults = testCases.some(t => !t.hasResults);
                            console.log(`📊 [CMP] hasTestsWithoutResults: ${hasTestsWithoutResults}`);
                            
                            if (hasTestsWithoutResults) {
                                console.log(`⚠️ [CMP] results가 없는 테스트 발견 - 브라우저 강제 종료 지점 탐지`);
                                parsedResults.status = 'fail';  // 브라우저 강제 종료로 fail 처리
                                console.log(`📊 [CMP] overallStatus를 fail로 변경`);
                                ...
                                console.log(`📊 [CMP] lastResultsIndex: ${lastResultsIndex}`);
                                
                                let failIndex = -1;
                                if (lastResultsIndex >= 0 && lastResultsIndex + 1 < testCases.length) {
                                    // 다음 테스트 (results 없는 첫 테스트)가 강제 종료 지점
                                    failIndex = lastResultsIndex + 1;
                                    console.log(`⚠️ [CMP] 브라우저 강제 종료 지점: ${testCases[failIndex].name} (인덱스: ${failIndex}, 마지막 results 있는 테스트 다음)`);
                                    ...
                                    console.log(`⚠️ [CMP] 브라우저 강제 종료 지점: ${testCases[failIndex].name} (인덱스: ${failIndex}, 마지막 테스트)`);
                                } else if (testCases.length > 0) {
                                    // results가 없는 테스트만 있는 경우, 첫 번째 테스트를 fail로
                                    failIndex = 0;
                                    console.log(`⚠️ [CMP] 브라우저 강제 종료 지점: ${testCases[failIndex].name} (인덱스: ${failIndex}, 첫 번째 테스트)`);
                                }
                                ...
                                if (failIndex >= 0) {
                                    ...
                                    console.log(`✅ [CMP] ${testCases[failIndex].name}을(를) fail로 변경 완료`);
                                    
                                    // 그 이후 테스트들은 이미 not-test로 되어 있음 (results 없는 테스트)
                                }
                                
                                console.log(`📊 [CMP] 브라우저 강제 종료 처리 완료: ${testCases.filter(t => t.status === 'pass').length}개 성공, ${testCases.filter(t => t.status === 'fail').length}개 실패, ${testCases.filter(t => t.status === 'not-test').length}개 미수행`);
                            }
                        }
                        
                    } else {
                        console.log(`⚠️ [CMP] 시나리오 ${scenarioId} 결과를 test-results.json에서 찾을 수 없음`);
                        parsedResults.status = 'fail';
                    }
                    
                } catch (parseError) {
                    console.log(`⚠️ [CMP] test-results.json 파싱 실패: ${parseError.message}`);
                    console.log(`📦 [CMP] 시나리오 파일에서 테스트 목록 추출 시도`);
                    ...
                    try {
                        ...
                        if (fs.existsSync(scenarioFilePath)) {
                            ...
                            console.log(`✅ [CMP] 시나리오 파일에서 ${extractedTests.length}개 테스트 추출`);
                            ...
                            if (parsedResults.testCases.length > 0) {
                                ...
                                console.log(`⚠️ [CMP] 첫 번째 테스트를 fail로 설정: ${parsedResults.testCases[0].name}`);
                            }
                        }
                    } catch (extractError) {
                        console.log(`❌ [CMP] 시나리오 파일 추출 실패: ${extractError.message}`);
                    }
                    
                    parsedResults.status = 'fail';
                }
            } else {
                console.log(`⚠️ [CMP] test-results.json 파일이 없음, 종료 코드로 상태 판단`);
                console.log(`📦 [CMP] 시나리오 파일에서 테스트 목록 추출 시도`);
            ...
            console.log(`✅ CMP 시나리오 ${scenarioId} 완료 플래그 생성: ${completeFlagPath}`);
            ...
            saveGlobalTestResults(scenarioId, testResults, 'CMP global-test-results.json 저장 완료');
            
            // ⚠️ testExitCode가 0이 아니면 실패로 종료 (VIOLA 방식)
            if (testExitCode !== 0) {
                console.log(`\n❌ CMP 시나리오 ${scenarioId} 실패로 종료 (testExitCode: ${testExitCode})`);
                process.exit(1);
            }
            
        } catch (error) {
            console.error('❌ CMP 커스텀 리포트 생성 실패:', error.message);
        }
        ...
        console.log('\n============================================================');
        console.log(`❌ CMP 시나리오 ${scenarioId} 실행 중 오류 발생`);
        console.log(`⏰ 종료 시간: ${endTime.toLocaleString('ko-KR')}`);
        console.log(`⏱️ 총 실행시간: ${durationFormatted} (${durationMs}ms)`);
        ...
        try {
            ...
            console.log('✅ CMP 실패 케이스 커스텀 리포트 생성 완료');
            
        } catch (reportError) {
            console.error('❌ CMP 실패 케이스 커스텀 리포트 생성 실패:', reportError.message);
        }
