...
test.describe.serial('시나리오 55: cmp test', () => {
  let page;
  let browser;
  let config;
  let managers;
  ...
  test.beforeAll(async ({ browser: newBrowser }) => {
    const result = await setupScenarioBeforeAll(newBrowser, 55, initializeManagers);
  ...
  test.afterAll(async () => {
    await setupScenarioAfterAll(page, browser, 55, '시나리오 55: cmp test', 'CMP');
  });
