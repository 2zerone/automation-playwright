...
class ReportGenerator {
    constructor(currentProduct = 'cmp') {
    ...
    loadTestSettings() {
        try {
            ...
            return {
                ...
                project: { code: "CMP", name: "kubernetes" },
                repository: { name: "CMP-kubernetes" },
                users: [],
                login: { userId: '' }
            };
        }
    ...
    getScenarioDescription(id) {
        ...
        // 제품별 기본 설명 제공
        if (this.currentProduct === 'cmp' && id === 1) {
            return 'CMP를 통해 생성한 클러스터가 사용가능한지 확인';
        } else if (this.currentProduct === 'cmp' && id === 2) {
    ...
    generateTestData(scenarioId, testResults) {
        ...
        const getScenarioName = (id) => {
            ...
            return cmpScenarioNames[id] || `시나리오 ${id}`;
        };
    ...
    generateCustomReport(scenarioId, reportData = null) {
        ...
        console.log(`🔍 CMP 시나리오 ${scenarioId}: 초기 testResults 상태:`, testResults ? '데이터 있음' : '데이터 없음');
        console.log(`🔍 CMP 시나리오 ${scenarioId}: reportData 전체:`, reportData);
        
        // testResults가 null인 경우 실시간 데이터 우선 사용
        if (!testResults) {
            console.log(`⚠️ 시나리오 ${scenarioId}: reportData에서 testResults를 찾을 수 없음. 실시간 데이터 확인 중...`);
            
            // 1. 먼저 CMP 전용 global.testResults Map에서 최신 데이터 확인
            let latestTestResults = null;
            if (global.testResults && global.testResults instanceof Map) {
                const scenarioKey = `cmp-scenario-${scenarioId}`;
                latestTestResults = global.testResults.get(scenarioKey);
                if (latestTestResults) {
                    console.log(`✅ CMP 시나리오 ${scenarioId}: global.testResults Map에서 최신 데이터 발견`);
                    console.log(`📊 CMP 시나리오 ${scenarioId}: Map 데이터 상태: ${latestTestResults.status}, 테스트 케이스: ${latestTestResults.testCases?.length || 0}개`);
                    testResults = latestTestResults;
                    reportData = reportData || {};
                    reportData.testResults = testResults;
                }
            } else {
                console.log(`⚠️ CMP 시나리오 ${scenarioId}: global.testResults Map이 존재하지 않음`);
            }
                ...
                try {
                    ...
                    if (fs.existsSync(globalTestResultsPath)) {
                        ...
                        if (testResultsData[scenarioKey]) {
                            ...
                            console.log(`✅ CMP 시나리오 ${scenarioId}: global-test-results.json에서 데이터 발견`);
                            
                            // testResults를 reportData에 설정
                            reportData = reportData || {};
                            reportData.testResults = testResults;
                        } else {
                            console.log(`⚠️ CMP 시나리오 ${scenarioId}: global-test-results.json에서 CMP 전용 데이터를 찾을 수 없음`);
                        }
                    } else {
            ...
            console.log(`✅ CMP 시나리오 ${scenarioId}: reportData에서 testResults 발견`);
            console.log(`📊 CMP 시나리오 ${scenarioId}: testResults 상태: ${testResults.status}, 테스트 케이스: ${testResults.testCases?.length || 0}개`);
        }
        
        // 강제로 실시간 데이터 우선 사용 (CMP 전용) - 캐시 무시
        if (global.testResults && global.testResults instanceof Map) {
            const scenarioKey = `cmp-scenario-${scenarioId}`;
            const latestTestResults = global.testResults.get(scenarioKey);
            if (latestTestResults) {
                console.log(`🔄 CMP 시나리오 ${scenarioId}: 캐시 무시하고 최신 데이터 강제 사용`);
                console.log(`📊 CMP 시나리오 ${scenarioId}: Map 최신 데이터 - 상태: ${latestTestResults.status}, 테스트 케이스: ${latestTestResults.testCases?.length || 0}개`);
                ...
                console.log(`✅ CMP 캐시 무시 완료: 새로운 타임스탬프 ${testResults.timestamp}`);
            }
        }
        ...
        // 우선순위: 3. testResults에서 전달된 데이터 사용 (CMP 동적 파싱 결과 우선 처리)
        if (testCases.length === 0 && testResults) {
            if (testResults.testCases && testResults.testCases.length > 0) {
                ...
                console.log(`📊 CMP 동적 파싱 결과 사용 - 빈 테스트 케이스 처리`);
                // 빈 테스트 케이스로 처리 (실제 결과 반영)
                testCases = [];
                scenarioStatus = testResults.status || 'fail';
                scenarioDuration = testResults.duration || 'N/A';
                console.log(`📊 CMP 동적 파싱 결과로 커스텀 리포트 생성 완료`);
        ...
        console.log(`📊 CMP 실시간 데이터 우선 사용 (파일 캐시 완전 무시)`);
        ...
        if (testResults && testResults.testCases !== undefined) {
            console.log(`✅ CMP 시나리오 ${scenarioId}: 실시간 testResults에서 데이터 발견`);
            console.log(`📊 CMP 실시간 상태: ${testResults.status}`);
            console.log(`📊 CMP 실시간 테스트 케이스 수: ${testResults.testCases.length}개`);
            ...
            console.log(`📊 CMP 실시간 데이터 사용 완료: ${testCases.length}개 테스트, 상태: ${scenarioStatus}`);
            console.log(`📊 CMP testCases 상세: ${testCases.map((tc, i) => `[${i}] ${tc.name}: ${tc.status}`).join(', ')}`);
            dynamicResultFound = true;
        } else {
            console.log(`⚠️ CMP 시나리오 ${scenarioId}: 실시간 testResults에서 데이터를 찾을 수 없음`);
            ...
            try {
                ...
                if (fs.existsSync(globalResultsPath)) {
                    ...
                    if (globalResults[scenarioKey]) {
                        console.log(`📊 CMP 파일에서 시나리오 ${scenarioId} 결과 발견 (fallback)`);
                        ...
                        console.log(`📊 CMP 파일 데이터 사용 (fallback): ${testCases.length}개 테스트, 상태: ${scenarioStatus}`);
                        dynamicResultFound = true;
                    }
                }
            } catch (error) {
                console.log(`❌ CMP 파일 읽기 실패: ${error.message}`);
            }
        }
        
        // 동적 결과를 찾았으면 다른 fallback 로직 건너뛰기
        if (dynamicResultFound) {
            console.log(`📊 CMP 동적 결과 사용 - fallback 로직 완전히 건너뛰기`);
        ...
        if (testCases.length === 0 && !dynamicResultFound) {
            ...
            if (testResults && testResults.testCases && testResults.testCases.length === 0) {
                ...
                console.log(`📊 CMP fallback 로직 건너뛰기 완료`);
...
<head>
    ...
    <title>CMP 자동화 테스트 커스텀 리포트 - ${currentScenarioName}</title>
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>CMP 자동화 테스트 마스터 리포트</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <style>
        * { 
            margin: 0; 
...
<body>
    <div class="container">
        <div class="header">
            <h1><i class="fas fa-music"></i> CMP 자동화 테스트</h1>
    ...
    mapScreenshotsToTestCases(testCases, scenarioId) {
        try {
            ...
            testCases.forEach(testCase => {
                ...
                // CMP/custom-reports/scenario-{scenarioId} 디렉토리에서 해당 테스트 케이스의 스크린샷 찾기
                const scenarioDir = path.join(__dirname, '..', 'custom-reports', `scenario-${scenarioId}`);
                const screenshots = [];
                ...
                const matchingScreenshots = screenshots.filter(screenshot => {
                    ...
                    // 기존 CMP 스타일: {stepName}_{status}_{timestamp}.png
                    const cleanTestCaseName = testCaseName.replace(/[^가-힣a-zA-Z0-9]/g, '');
